...
app.get('/style.css', function(req, res) {
	res.sendFile(path.resolve(__dirname+'/website/style.css'));
});

const logger = new (winston.Logger)({
	transports: [
		new winston.transports.Console({timestamp:(new Date()).toLocaleTimeString(),colorize:true,level:'info'}),
		new winston.transports.File({name:'a',json:false,filename:'logfile.txt',timestamp:(new Date()).toLocaleTimeString(),level:'error'}),
	]
});
...
const workerserver = net.createServer(function (localsocket) {
	...
	localsocket.on('data', function (data) {
		try{
			...
			logger.warn('jso error before login '+e.message);
		}
	});
...
io.on('connection', function(socket){
	...
	socket.on('reload',function(user, password) {
		config = JSON.parse(fs.readFileSync('config.json'));
		pools = config.pools;
		
		if(password!=config.reloadpass)
		{
			socket.emit('msg', {type: "danger", text: "Failed to reload config, reload password didn't match!"});
			logger.error("Pool reload config failed, wrong password ("+user+")");
			return;
		}
		...
		socket.emit('msg', {type: "success", text: "Pool config reloaded"});
		logger.info("pool config reloaded");
	});
	socket.on('user',function(user) {
		if(pools[user] === undefined)
		{
			socket.emit('msg', {type: "warning", text: "User "+user+" not found"});
			return;
		}
